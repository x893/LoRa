...
void Suspend(void)
{
	...
	for (i = 0; i < 8; i++)
		pInformation->EP[i] = _GetENDPOINT(i);
	...
	/* restore Enpoints */
	for (i = 0; i < 8; i++)
		_SetENDPOINT(i, pInformation->EP[i]);
	...
	PWR->CR = (savePWR_CR & ~(uint32_t)(PWR_CR_LPSDSR | PWR_CR_PDDS)) | PWR_Regulator_LowPower;
...
void Resume(RESUME_STATE eResumeSetVal)
{
	...
	case RESUME_EXTERNAL:
		if (pInformation->remotewakeupon == false)
		{
			Resume_Init();
	...
	case RESUME_INTERNAL:
		...
		pInformation->remotewakeupon = true;
		break;
	case RESUME_LATER:
		ResumeS.bESOFcnt = 2;
	...
	case RESUME_ON:	
		...
		if (ResumeS.bESOFcnt == 0)
		{
			...
			_SetCNTR(wCNTR);
			ResumeS.eState = RESUME_OFF;
			pInformation->remotewakeupon = false;
		}
		break;
	case RESUME_OFF:
	case RESUME_ESOF:
