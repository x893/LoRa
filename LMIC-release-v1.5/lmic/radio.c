...
// ---------------------------------------- 
// Registers Mapping
#define LORA_REG_00_FIFO							0x00 // common
#define LORA_REG_01_OP_MODE						0x01 // common
...
#define LORALORA_REG_00_FIFOTxBaseAddr			0x0E
#define FSKRegRssiCollision				0x0F
#define LORALORA_REG_00_FIFORxBaseAddr		0x0F 
...
#define LORALORA_REG_00_FIFORxByteAddr			0x25
...
#define FSKLORA_REG_00_FIFOThresh				0x35
...
{
	writeReg(LORA_REG_01_OP_MODE, (readReg(LORA_REG_01_OP_MODE) & ~OPMODE_MASK) | mode);
}
...
#endif
	writeReg(LORA_REG_01_OP_MODE, u);
}
...
static void configLoraModem ()
{
	...
	// set ModemConfig1
	writeReg(LORA_REG_1D_MODEM_CONFIG1, mc1);
...
static void txfsk ()
{
	...
	writeReg(LORA_REG_01_OP_MODE, 0x10); // FSK, BT=0.5
	ASSERT(readReg(LORA_REG_01_OP_MODE) == 0x10);
	...
	writeReg(LORA_REG_00_FIFO, LMIC.dataLen);
	writeBuf(LORA_REG_00_FIFO, LMIC.frame, LMIC.dataLen);
...
static void txlora ()
{
	...
	// writeReg(LORA_REG_01_OP_MODE, OPMODE_LORA);
	opmodeLora();
	ASSERT((readReg(LORA_REG_01_OP_MODE) & OPMODE_LORA) != 0);
	...
	writeReg(LORA_REG_12_IRQ_FLAGS, 0xFF);
	// mask all IRQs but TxDone
	writeReg(LORA_REG_11_IRQ_FLAGS_MASK, ~IRQ_LORA_TXDONE_MASK);

	// initialize the payload size and address pointers	
	writeReg(LORALORA_REG_00_FIFOTxBaseAddr, 0x00);
	writeReg(LORA_REG_0D_FIFO_ADDR_PTR, 0x00);
...
static void starttx ()
{
	ASSERT( (readReg(LORA_REG_01_OP_MODE) & OPMODE_MASK) == OPMODE_SLEEP );
...
static void rxlora (uint8_t rxmode)
{
	...
	{	// use fixed settings for rssi scan
		writeReg(LORA_REG_1D_MODEM_CONFIG1, RXLORA_RXMODE_RSSI_REG_MODEM_CONFIG1);
	...
	writeReg(LORA_REG_11_IRQ_FLAGS_MASK, ~rxlorairqmask[rxmode]);
...
static void rxfsk (uint8_t rxmode)
{
	...
	// select FSK modem (from sleep mode)
	//writeReg(LORA_REG_01_OP_MODE, 0x00); // (not LoRa)
	opmodeFSK();
	ASSERT((readReg(LORA_REG_01_OP_MODE) & OPMODE_LORA) == 0);
	// enter standby mode (warm up))
	opmode(OPMODE_STANDBY);
...
void radio_init ()
{
	...
	rxlora(RXMODE_RSSI);
	while ( (readReg(LORA_REG_01_OP_MODE) & OPMODE_MASK) != OPMODE_RX )
		; // continuous rx
	for (int i = 1; i < 16; i++)
	{
		for (int j = 0; j < 8; j++)
...
void radio_irq_handler (uint8_t dio)
{
	...
	if ( (readReg(LORA_REG_01_OP_MODE) & OPMODE_LORA) != 0)
	{	// LORA modem
		uint8_t flags = readReg(LORA_REG_12_IRQ_FLAGS);
		...
		else if ( flags & IRQ_LORA_RXDONE_MASK )
		{
			...
			LMIC.dataLen = (readReg(LORA_REG_1D_MODEM_CONFIG1) & SX1272_MC1_IMPLICIT_HEADER_MODE_ON)
							? readReg(LORARegPayloadLength)
							: readReg(LORARegRxNbBytes);
			// set FIFO read address pointer
			writeReg(LORA_REG_0D_FIFO_ADDR_PTR, readReg(LORA_REG_10_FIFO_RX_CURRENT_ADDR));
			// now read the FIFO
			readBuf(LORA_REG_00_FIFO, LMIC.frame, LMIC.dataLen);
			// read rx quality parameters
			LMIC.snr  = readReg(LORA_REG_19_PKT_SNR_VALUE); // SNR [dB] * 4
			LMIC.rssi = readReg(LORA_REG_1A_PKT_RSSI_VALUE) - 125 + 64; // RSSI [dBm] (-196...+63)
		}
		...
		// mask all radio IRQs
		writeReg(LORA_REG_11_IRQ_FLAGS_MASK, 0xFF);
