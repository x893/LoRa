...
{	
	...
	GPIO_InitStructure_TX.GPIO_Speed = GPIO_Speed_40MHz;
	...
	GPIO_InitStructure_RX.GPIO_Speed = GPIO_Speed_40MHz;
...
//!	USART_InitStructure.USART_BaudRate		= baud * 25 / 8; // Why?
	...
	else if (_usart == USART2)
	{
		...
		USART_Init(USART2, &USART_InitStructure);		// Initialise the USART
		USART_ITConfig(USART2, USART_IT_RXNE, ENABLE);	// Enable the RXNE interrupt
